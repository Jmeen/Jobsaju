...
test('모바일 결제 리디렉션에 필요한 결제 문맥을 탭 세션에 보관한다', () => {
  savePendingPayment({ paymentId: 'p-123', email: 'user@example.com', couponCode: 'SALE30' });
  assert.deepEqual(loadPendingPayment(), { paymentId: 'p-123', email: 'user@example.com', couponCode: 'SALE30' });
  clearPendingPayment();

...
test('유료 공유물은 점수·추천 시기 대신 수호신 정보만 담는다', () => {
  ...
  assert.equal(message.title, buildGuardianShareMessage(tiger).title);
  assert.equal(message.question, '너의 수호신은 누구일까?');
  assert.doesNotMatch(JSON.stringify(message), /이직|협상|점수|10월|71/);
});

test('유료 템플릿 인자에도 수호신 문구만 들어간다', () => {
  const message = buildPaidReportShareMessage(
    tiger,
    { jobChange: 52, stay: 59, negotiation: 53 },
    { best_job_change: { year_month: '2026-10', score: 71 } },
  ...
  assert.equal(args.SHARE_TITLE, buildGuardianShareMessage(tiger).title);
  assert.equal(args.SHARE_QUESTION, '너의 수호신은 누구일까?');
  assert.doesNotMatch(`${args.SHARE_TITLE} ${args.SHARE_QUESTION} ${args.GUARDIAN_DESCRIPTION}`, /이직운|협상운|10월|71/);
});
